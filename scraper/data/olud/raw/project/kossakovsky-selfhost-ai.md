# selfhost-ai — 🚀 Self-hosted AI automation platform. Deploy n8n, Ol… | olud.ai

> Set up over 30 AI and automation tools on your own server with just one command, including workflow automation and (Apache-2.0, 2025). 921 stars on GitHub.

_Source: https://olud.ai/project/kossakovsky-selfhost-ai.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
selfhost-ai

selfhost-ai

[Shell](https://olud.ai/language/shell.html)

## selfhost-ai

🚀 Self-hosted AI automation platform. Deploy n8n, Ollama, Flowise, RAG, Supabase & 30+ tools with one command. Auto HTTPS. Free Zapier/Make alternative.

by **kossakovsky** · GitHub

[921](https://olud.ai/most-starred.html)

Stars

[233](https://olud.ai/most-forked.html)

Forks

[+5 this week](https://olud.ai/trending.html)

Trending

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2025](https://olud.ai/year/2025.html)

Created

[3 days ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Shell](https://olud.ai/language/shell.html)

Language

Likely

Self-hostable
- [ai](https://olud.ai/projects.html?cat=ai)
- [ai-agents](https://olud.ai/projects.html?cat=ai-agents)
- [automation](https://olud.ai/projects.html?cat=automation)
Apache-2.0
- [Shell](https://olud.ai/language/shell.html)
Self-hostable

[View on GitHub](https://github.com/kossakovsky/selfhost-ai)

𝕏  X / Twitter

🔗  Copy link

In plain wordsSet up over 30 AI and automation tools on your own server with just one command, including workflow automation and databases.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=kossakovsky%2Fselfhost-ai)

📈 Star history

2026-07-202026-08-31

📈 Track selfhost-ai

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

🚀 Self-hosted AI automation platform. Deploy n8n, Ollama, Flowise, RAG, Supabase & 30+ tools with one command. Auto HTTPS. Free Zapier/Make alternative.

selfhost-ai has 921 stars on GitHub. It has been forked 233 times. selfhost-ai is written mainly in Shell. It has been in active development since 2025. selfhost-ai is available under the Apache-2.0 license. Its main topics are ai, ai-agents, automation, chatgpt-alternative.

[Compare with the paid tools it replaces](https://olud.ai/alt/selfhost-ai.html)A free alternative to 
- [Zapier AI](https://olud.ai/alternatives/zapier-ai.html)

📦 Latest release

❓ Frequently asked questions

#### What is selfhost-ai?
🚀 Self-hosted AI automation platform. Deploy n8n, Ollama, Flowise, RAG, Supabase & 30+ tools with one command. Auto HTTPS. Free Zapier/Make alternative.

#### Is selfhost-ai open source?
selfhost-ai is an open-source project. It is released under the Apache-2.0 license.

#### Is selfhost-ai free?
Yes. selfhost-ai is free and open source — you can use, modify and self-host it.

#### What license does selfhost-ai use?
selfhost-ai is available under the Apache-2.0 license.

#### What language is selfhost-ai written in?
selfhost-ai is written mainly in Shell.

🏅 Maintainer of this project?

olud.ai badge — selfhost-ai
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=kossakovsky-selfhost-ai)](https://olud.ai/project/kossakovsky-selfhost-ai.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/dify.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
