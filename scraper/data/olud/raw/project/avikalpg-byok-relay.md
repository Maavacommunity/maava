# byok-relay — Bring your own key (BYOK) relay to enable lightweigh… | olud.ai

> Allow users to utilize their own AI keys directly from your web app without any coding issues (Apache-2.0, 2026). 54 stars on GitHub.

_Source: https://olud.ai/project/avikalpg-byok-relay.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
byok-relay

byok-relay

[JavaScript](https://olud.ai/language/javascript.html)

## byok-relay

Bring your own key (BYOK) relay to enable lightweight frontend-only apps to use LLMs using the user's API keys without hitting CORS.

by **avikalpg** · GitHub

[54](https://olud.ai/most-starred.html)

Stars

[1](https://olud.ai/most-forked.html)

Forks

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2026](https://olud.ai/year/2026.html)

Created

[today](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[JavaScript](https://olud.ai/language/javascript.html)

Language

Likely

Self-hostable
- [ai](https://olud.ai/projects.html?cat=ai)
- [ai-gateway](https://olud.ai/dna/ai-gateway.html)
- [anthropic](https://olud.ai/projects.html?cat=anthropic)
Apache-2.0
- [JavaScript](https://olud.ai/language/javascript.html)
Self-hostable

[View on GitHub](https://github.com/avikalpg/byok-relay)

𝕏  X / Twitter

🔗  Copy link

In plain wordsAllow users to utilize their own AI keys directly from your web app without any coding issues.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=avikalpg%2Fbyok-relay)

📈 Star history

2026-07-202026-08-31

📈 Track byok-relay

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

Bring your own key (BYOK) relay to enable lightweight frontend-only apps to use LLMs using the user's API keys without hitting CORS.

byok-relay has 54 stars on GitHub. It has been forked 1 times. byok-relay is written mainly in JavaScript. It has been in active development since 2026. byok-relay is available under the Apache-2.0 license. Its main topics are ai, ai-gateway, anthropic, api-proxy.

📦 Latest release

❓ Frequently asked questions

#### What is byok-relay?
Bring your own key (BYOK) relay to enable lightweight frontend-only apps to use LLMs using the user's API keys without hitting CORS.

#### Is byok-relay open source?
byok-relay is an open-source project. It is released under the Apache-2.0 license.

#### Is byok-relay free?
Yes. byok-relay is free and open source — you can use, modify and self-host it.

#### What license does byok-relay use?
byok-relay is available under the Apache-2.0 license.

#### What language is byok-relay written in?
byok-relay is written mainly in JavaScript.

🏅 Maintainer of this project?

olud.ai badge — byok-relay
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=avikalpg-byok-relay)](https://olud.ai/project/avikalpg-byok-relay.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Shares DNA with[🧬 View the DNA map →](https://olud.ai/dna/ai-gateway.html)
- [ccLoad AI API gateway that ends manual channel switching with smart routing, auto failover, exponentia… ★ 361 · ai sharesapi-proxyai-gateway](https://olud.ai/project/caidaoli-ccload.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)

Measured from GitHub topics shared by both projects, weighted by how rare each topic is.
