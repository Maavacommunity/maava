# EmotiVoice — EmotiVoice 😊: a Multi-Voice and Prompt-Controlled TT… | olud.ai

> Create speech with different emotions using a text-to-speech engine that supports multiple voices (Apache-2.0, 2023). 8.5k stars on GitHub.

_Source: https://olud.ai/project/netease-youdao-emotivoice.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
EmotiVoice

EmotiVoice

[Python](https://olud.ai/language/python.html)

## EmotiVoice

EmotiVoice 😊: a Multi-Voice and Prompt-Controlled TTS Engine

by **netease-youdao** · GitHub

[8.5k](https://olud.ai/most-starred.html)

Stars

[754](https://olud.ai/most-forked.html)

Forks

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2023](https://olud.ai/year/2023.html)

Created

[2 yr ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Python](https://olud.ai/language/python.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [deep-learning](https://olud.ai/projects.html?cat=deep-learning)
- [emotion](https://olud.ai/projects.html?cat=emotion)
Apache-2.0
- [Python](https://olud.ai/language/python.html)

[View on GitHub](https://github.com/netease-youdao/EmotiVoice)

𝕏  X / Twitter

🔗  Copy link

In plain wordsCreate speech with different emotions using a text-to-speech engine that supports multiple voices.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=netease-youdao%2FEmotiVoice)

📈 Star history

2026-07-042026-08-31

📈 Track EmotiVoice

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

EmotiVoice 😊: a Multi-Voice and Prompt-Controlled TTS Engine

EmotiVoice has 8.5k stars on GitHub. It has been forked 754 times. EmotiVoice is written mainly in Python. It has been in active development since 2023. EmotiVoice is available under the Apache-2.0 license. Its main topics are ai, deep-learning, emotion, emotivoice.

📦 Latest release

❓ Frequently asked questions

#### What is EmotiVoice?
EmotiVoice 😊: a Multi-Voice and Prompt-Controlled TTS Engine

#### Is EmotiVoice open source?
EmotiVoice is an open-source project. It is released under the Apache-2.0 license.

#### Is EmotiVoice free?
Yes. EmotiVoice is free and open source — you can use, modify and self-host it.

#### What license does EmotiVoice use?
EmotiVoice is available under the Apache-2.0 license.

#### What language is EmotiVoice written in?
EmotiVoice is written mainly in Python.

🏅 Maintainer of this project?

olud.ai badge — EmotiVoice
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=netease-youdao-emotivoice)](https://olud.ai/project/netease-youdao-emotivoice.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Shares DNA with
- [EmoGen PyTorch Implementation for Paper "Emotionally Enhanced Talking Face Generation" (ICCVW'23 and A… ★ 379 · audio-driven-talking-face sharesemotion](https://olud.ai/project/sahilg06-emogen.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)

Measured from GitHub topics shared by both projects, weighted by how rare each topic is.
