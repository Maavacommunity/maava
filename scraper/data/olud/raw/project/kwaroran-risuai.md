# Risuai — Make your own story. User-friendly software for LLM… | olud.ai

> Engage in roleplaying and storytelling with an AI chat application that supports various features and APIs (GPL-3.0, 2023). 1.6k stars on GitHub.

_Source: https://olud.ai/project/kwaroran-risuai.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
Risuai

Risuai

[TypeScript](https://olud.ai/language/typescript.html)

## Risuai

Make your own story. User-friendly software for LLM roleplaying

by **kwaroran** · GitHub

[1.6k](https://olud.ai/most-starred.html)

Stars

[341](https://olud.ai/most-forked.html)

Forks

[+30 this week](https://olud.ai/trending.html)

Trending

[GPL-3.0](https://olud.ai/license/gpl-3-0.html)

License

[2023](https://olud.ai/year/2023.html)

Created

[yesterday](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[TypeScript](https://olud.ai/language/typescript.html)

Language

Likely

Self-hostable
- [ai](https://olud.ai/projects.html?cat=ai)
- [characters](https://olud.ai/projects.html?cat=characters)
- [chat](https://olud.ai/dna/chat.html)
GPL-3.0
- [TypeScript](https://olud.ai/language/typescript.html)
Self-hostable

[View on GitHub](https://github.com/kwaroran/Risuai)

𝕏  X / Twitter

🔗  Copy link

In plain wordsEngage in roleplaying and storytelling with an AI chat application that supports various features and APIs.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=kwaroran%2FRisuai)

📈 Star history

2026-07-202026-08-31

📈 Track Risuai

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

Make your own story. User-friendly software for LLM roleplaying

Risuai has 1.6k stars on GitHub. It has been forked 341 times. Risuai is written mainly in TypeScript. It has been in active development since 2023. Risuai is available under the GPL-3.0 license. Its main topics are ai, characters, chat, chatbot.

📦 Latest release

❓ Frequently asked questions

#### What is Risuai?
Make your own story. User-friendly software for LLM roleplaying

#### Is Risuai open source?
Risuai is an open-source project. It is released under the GPL-3.0 license.

#### Is Risuai free?
Yes. Risuai is free and open source — you can use, modify and self-host it. Its GPL-3.0 license is copyleft: if you distribute a modified version, it must remain under the same license.

#### What license does Risuai use?
Risuai is available under the GPL-3.0 license.

#### What language is Risuai written in?
Risuai is written mainly in TypeScript.

🏅 Maintainer of this project?

olud.ai badge — Risuai
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=kwaroran-risuai)](https://olud.ai/project/kwaroran-risuai.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/chat.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
