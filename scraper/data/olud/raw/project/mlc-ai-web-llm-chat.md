# web-llm-chat — Chat with AI large language models running natively… | olud.ai

> Chat privately with AI models directly in your web browser without needing a server (Apache-2.0, 2024). 1.1k stars on GitHub.

_Source: https://olud.ai/project/mlc-ai-web-llm-chat.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
web-llm-chat

web-llm-chat

[TypeScript](https://olud.ai/language/typescript.html)

## web-llm-chat

Chat with AI large language models running natively in your browser. Enjoy private, server-free, seamless AI conversations.

by **mlc-ai** · GitHub

[1.1k](https://olud.ai/most-starred.html)

Stars

[228](https://olud.ai/most-forked.html)

Forks

[+7 this week](https://olud.ai/trending.html)

Trending

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2024](https://olud.ai/year/2024.html)

Created

[6 mo ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[TypeScript](https://olud.ai/language/typescript.html)

Language

Likely

Self-hostable
- [ai](https://olud.ai/projects.html?cat=ai)
- [chat](https://olud.ai/dna/chat.html)
- [chat-application](https://olud.ai/dna/chat-application.html)
Apache-2.0
- [TypeScript](https://olud.ai/language/typescript.html)
Self-hostable

[View on GitHub](https://github.com/mlc-ai/web-llm-chat)

𝕏  X / Twitter

🔗  Copy link

In plain wordsChat privately with AI models directly in your web browser without needing a server.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=mlc-ai%2Fweb-llm-chat)

📈 Star history

2026-07-202026-08-31

📈 Track web-llm-chat

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

Chat with AI large language models running natively in your browser. Enjoy private, server-free, seamless AI conversations.

web-llm-chat has 1.1k stars on GitHub. It has been forked 228 times. web-llm-chat is written mainly in TypeScript. It has been in active development since 2024. web-llm-chat is available under the Apache-2.0 license. Its main topics are ai, chat, chat-application, chatbot.

📦 Latest release

❓ Frequently asked questions

#### What is web-llm-chat?
Chat with AI large language models running natively in your browser. Enjoy private, server-free, seamless AI conversations.

#### Is web-llm-chat open source?
web-llm-chat is an open-source project. It is released under the Apache-2.0 license.

#### Is web-llm-chat free?
Yes. web-llm-chat is free and open source — you can use, modify and self-host it.

#### What license does web-llm-chat use?
web-llm-chat is available under the Apache-2.0 license.

#### What language is web-llm-chat written in?
web-llm-chat is written mainly in TypeScript.

🏅 Maintainer of this project?

olud.ai badge — web-llm-chat
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=mlc-ai-web-llm-chat)](https://olud.ai/project/mlc-ai-web-llm-chat.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/chat.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
