# spine-animation-ai — AI-powered Claude skill for Spine 2D skeletal animat… | olud.ai

> Create and animate 2D characters using AI, turning raw assets into interactive previews without needing to redraw or re-rig (—, 2026). 388 stars on GitHub.

_Source: https://olud.ai/project/genielabsopensource-spine-animation-ai.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
spine-animation-ai

spine-animation-ai

[Python](https://olud.ai/language/python.html)

## spine-animation-ai

AI-powered Claude skill for Spine 2D skeletal animation — auto-rig, animate, and preview characters

by **GenielabsOpenSource** · GitHub

[388](https://olud.ai/most-starred.html)

Stars

[59](https://olud.ai/most-forked.html)

Forks

[+45 this week](https://olud.ai/trending.html)

Trending

[2026](https://olud.ai/year/2026.html)

Created

[4 days ago](https://olud.ai/recently-updated.html)

Last commit

[2d-animation](https://olud.ai/projects.html?cat=2d-animation)

Category

[Python](https://olud.ai/language/python.html)

Language
- [2d-animation](https://olud.ai/projects.html?cat=2d-animation)
- [ai](https://olud.ai/projects.html?cat=ai)
- [animation](https://olud.ai/dna/animation.html)
- [Python](https://olud.ai/language/python.html)

[View on GitHub](https://github.com/GenielabsOpenSource/spine-animation-ai)

𝕏  X / Twitter

🔗  Copy link

In plain wordsCreate and animate 2D characters using AI, turning raw assets into interactive previews without needing to redraw or re-rig.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=GenielabsOpenSource%2Fspine-animation-ai)

📈 Star history

2026-07-202026-08-31

📈 Track spine-animation-ai

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

AI-powered Claude skill for Spine 2D skeletal animation — auto-rig, animate, and preview characters

spine-animation-ai has 388 stars on GitHub. It has been forked 59 times. spine-animation-ai is written mainly in Python. It has been in active development since 2026. Its main topics are 2d-animation, ai, animation, animation-tool.

📦 Latest release

❓ Frequently asked questions

#### What is spine-animation-ai?
AI-powered Claude skill for Spine 2D skeletal animation — auto-rig, animate, and preview characters

#### Is spine-animation-ai open source?
spine-animation-ai is an open-source project.

#### Is spine-animation-ai free?
Yes. spine-animation-ai is free and open source — you can use, modify and self-host it.

#### What language is spine-animation-ai written in?
spine-animation-ai is written mainly in Python.

🏅 Maintainer of this project?

olud.ai badge — spine-animation-ai
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=genielabsopensource-spine-animation-ai)](https://olud.ai/project/genielabsopensource-spine-animation-ai.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/animation.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
