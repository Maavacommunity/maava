# RobustVideoMatting — Robust Video Matting in PyTorch, TensorFlow, TensorF… | olud.ai

> Perform real-time video editing to isolate people from backgrounds in any video without needing extra inputs (GPL-3.0, 2021). 9.4k stars on GitHub.

_Source: https://olud.ai/project/peterl1n-robustvideomatting.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
RobustVideoMatting

RobustVideoMatting

[Python](https://olud.ai/language/python.html)

## RobustVideoMatting

Robust Video Matting in PyTorch, TensorFlow, TensorFlow.js, ONNX, CoreML!

by **PeterL1n** · GitHub

[9.4k](https://olud.ai/most-starred.html)

Stars

[1.2k](https://olud.ai/most-forked.html)

Forks

[GPL-3.0](https://olud.ai/license/gpl-3-0.html)

License

[2021](https://olud.ai/year/2021.html)

Created

[2 yr ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Python](https://olud.ai/language/python.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [computer-vision](https://olud.ai/projects.html?cat=computer-vision)
- [deep-learning](https://olud.ai/projects.html?cat=deep-learning)
GPL-3.0
- [Python](https://olud.ai/language/python.html)

[View on GitHub](https://github.com/PeterL1n/RobustVideoMatting)

𝕏  X / Twitter

🔗  Copy link

In plain wordsPerform real-time video editing to isolate people from backgrounds in any video without needing extra inputs.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=PeterL1n%2FRobustVideoMatting)

📈 Star history

2026-07-042026-08-31

📈 Track RobustVideoMatting

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

Robust Video Matting in PyTorch, TensorFlow, TensorFlow.js, ONNX, CoreML!

RobustVideoMatting has 9.4k stars on GitHub. It has been forked 1.2k times. RobustVideoMatting is written mainly in Python. It has been in active development since 2021. RobustVideoMatting is available under the GPL-3.0 license. Its main topics are ai, computer-vision, deep-learning, machine-learning.

📦 Latest release

❓ Frequently asked questions

#### What is RobustVideoMatting?
Robust Video Matting in PyTorch, TensorFlow, TensorFlow.js, ONNX, CoreML!

#### Is RobustVideoMatting open source?
RobustVideoMatting is an open-source project. It is released under the GPL-3.0 license.

#### Is RobustVideoMatting free?
Yes. RobustVideoMatting is free and open source — you can use, modify and self-host it. Its GPL-3.0 license is copyleft: if you distribute a modified version, it must remain under the same license.

#### What license does RobustVideoMatting use?
RobustVideoMatting is available under the GPL-3.0 license.

#### What language is RobustVideoMatting written in?
RobustVideoMatting is written mainly in Python.

🏅 Maintainer of this project?

olud.ai badge — RobustVideoMatting
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=peterl1n-robustvideomatting)](https://olud.ai/project/peterl1n-robustvideomatting.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
