# EvalAI — :cloud: :rocket: :bar_chart: :chart_with_upwards_tre… | olud.ai

> Evaluate and compare different AI algorithms using a centralized platform for submissions and leaderboards (—, 2016). 2k stars on GitHub.

_Source: https://olud.ai/project/cloud-cv-evalai.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
EvalAI

EvalAI

[Python](https://olud.ai/language/python.html)

## EvalAI

:cloud: :rocket: :bar_chart: :chart_with_upwards_trend: Evaluating state of the art in AI

by **Cloud-CV** · GitHub

[2k](https://olud.ai/most-starred.html)

Stars

[983](https://olud.ai/most-forked.html)

Forks

[2016](https://olud.ai/year/2016.html)

Created

[1 mo ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Python](https://olud.ai/language/python.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [ai-challenges](https://olud.ai/projects.html?cat=ai-challenges)
- [angularjs](https://olud.ai/projects.html?cat=angularjs)
- [Python](https://olud.ai/language/python.html)

[View on GitHub](https://github.com/Cloud-CV/EvalAI)

𝕏  X / Twitter

🔗  Copy link

In plain wordsEvaluate and compare different AI algorithms using a centralized platform for submissions and leaderboards.

✓ Claimed by its maintainer

📈 Star history

2026-07-072026-08-31

📈 Track EvalAI

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

:cloud: :rocket: :bar_chart: :chart_with_upwards_trend: Evaluating state of the art in AI

EvalAI has 2k stars on GitHub. It has been forked 983 times. EvalAI is written mainly in Python. It has been in active development since 2016. Its main topics are ai, ai-challenges, angularjs, artificial-intelligence.

📦 Latest release

❓ Frequently asked questions

#### What is EvalAI?
:cloud: :rocket: :bar_chart: :chart_with_upwards_trend: Evaluating state of the art in AI

#### Is EvalAI open source?
EvalAI is an open-source project.

#### Is EvalAI free?
Yes. EvalAI is free and open source — you can use, modify and self-host it.

#### What language is EvalAI written in?
EvalAI is written mainly in Python.

🏅 Maintainer of this project?

olud.ai badge — EvalAI
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=cloud-cv-evalai)](https://olud.ai/project/cloud-cv-evalai.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Shares DNA with
- [Deep-Learning-in-Production In this repository, I will share some useful notes and references about deploying deep learning… ★ 4.4k · angularjs sharesangularjs](https://olud.ai/project/ahkarami-deep-learning-in-production.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)

Measured from GitHub topics shared by both projects, weighted by how rare each topic is.
