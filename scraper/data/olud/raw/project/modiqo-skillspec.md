# skillspec — SkillSpec makes agent skills followable, testable, a… | olud.ai

> Check if your AI agent follows specific rules and generate reports on its performance with a single command (Apache-2.0, 2026). 739 stars on GitHub.

_Source: https://olud.ai/project/modiqo-skillspec.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
skillspec

skillspec

[Rust](https://olud.ai/language/rust.html)

## skillspec

SkillSpec makes agent skills followable, testable, and provable with Doctor risk reports, guided imports, structured contracts, and alignment proof.

by **modiqo** · GitHub

[739](https://olud.ai/most-starred.html)

Stars

[57](https://olud.ai/most-forked.html)

Forks

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2026](https://olud.ai/year/2026.html)

Created

[21 days ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Rust](https://olud.ai/language/rust.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [ai-agents](https://olud.ai/projects.html?cat=ai-agents)
- [ai-evals](https://olud.ai/projects.html?cat=ai-evals)
Apache-2.0
- [Rust](https://olud.ai/language/rust.html)

[View on GitHub](https://github.com/modiqo/skillspec)

𝕏  X / Twitter

🔗  Copy link

In plain wordsCheck if your AI agent follows specific rules and generate reports on its performance with a single command.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=modiqo%2Fskillspec)

📈 Star history

2026-07-202026-08-31

📈 Track skillspec

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

SkillSpec makes agent skills followable, testable, and provable with Doctor risk reports, guided imports, structured contracts, and alignment proof.

skillspec has 739 stars on GitHub. It has been forked 57 times. skillspec is written mainly in Rust. It has been in active development since 2026. skillspec is available under the Apache-2.0 license. Its main topics are ai, ai-agents, ai-evals, ai-tool.

📦 Latest release

❓ Frequently asked questions

#### What is skillspec?
SkillSpec makes agent skills followable, testable, and provable with Doctor risk reports, guided imports, structured contracts, and alignment proof.

#### Is skillspec open source?
skillspec is an open-source project. It is released under the Apache-2.0 license.

#### Is skillspec free?
Yes. skillspec is free and open source — you can use, modify and self-host it.

#### What license does skillspec use?
skillspec is available under the Apache-2.0 license.

#### What language is skillspec written in?
skillspec is written mainly in Rust.

🏅 Maintainer of this project?

olud.ai badge — skillspec
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=modiqo-skillspec)](https://olud.ai/project/modiqo-skillspec.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/ai-tool.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
