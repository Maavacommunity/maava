# chatgpt-i18n — Localize your websites with AI assistance. | olud.ai

> Translate your website or application files into different languages using AI assistance for better accuracy (AGPL-3.0, 2023). 807 stars on GitHub.

_Source: https://olud.ai/project/observedobserver-chatgpt-i18n.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
chatgpt-i18n

chatgpt-i18n

[TypeScript](https://olud.ai/language/typescript.html)

## chatgpt-i18n

Localize your websites with AI assistance.

by **ObservedObserver** · GitHub

[807](https://olud.ai/most-starred.html)

Stars

[99](https://olud.ai/most-forked.html)

Forks

[AGPL-3.0](https://olud.ai/license/agpl-3-0.html)

License

[2023](https://olud.ai/year/2023.html)

Created

[2 yr ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[TypeScript](https://olud.ai/language/typescript.html)

Language

Likely

Self-hostable
- [ai](https://olud.ai/projects.html?cat=ai)
- [chatgpt](https://olud.ai/projects.html?cat=chatgpt)
- [chatgpt-i18n](https://olud.ai/projects.html?cat=chatgpt-i18n)
AGPL-3.0
- [TypeScript](https://olud.ai/language/typescript.html)
Self-hostable

[View on GitHub](https://github.com/ObservedObserver/chatgpt-i18n)

𝕏  X / Twitter

🔗  Copy link

In plain wordsTranslate your website or application files into different languages using AI assistance for better accuracy.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=ObservedObserver%2Fchatgpt-i18n)

📈 Star history

2026-07-202026-08-31

📈 Track chatgpt-i18n

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

Localize your websites with AI assistance.

chatgpt-i18n has 807 stars on GitHub. It has been forked 99 times. chatgpt-i18n is written mainly in TypeScript. It has been in active development since 2023. chatgpt-i18n is available under the AGPL-3.0 license. Its main topics are ai, chatgpt, chatgpt-i18n, frontend.

📦 Latest release

❓ Frequently asked questions

#### What is chatgpt-i18n?
Localize your websites with AI assistance.

#### Is chatgpt-i18n open source?
chatgpt-i18n is an open-source project. It is released under the AGPL-3.0 license.

#### Is chatgpt-i18n free?
Yes. chatgpt-i18n is free and open source — you can use, modify and self-host it. Its AGPL-3.0 license is a strong copyleft: if you distribute a modified version — including offering it as a network service — your changes must be released under the same license.

#### What license does chatgpt-i18n use?
chatgpt-i18n is available under the AGPL-3.0 license.

#### What language is chatgpt-i18n written in?
chatgpt-i18n is written mainly in TypeScript.

🏅 Maintainer of this project?

olud.ai badge — chatgpt-i18n
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=observedobserver-chatgpt-i18n)](https://olud.ai/project/observedobserver-chatgpt-i18n.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
