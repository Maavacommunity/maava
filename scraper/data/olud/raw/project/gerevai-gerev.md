# gerev — 🧠 AI-powered enterprise search engine 🔎 | olud.ai

> Search for documents and conversations within your organization quickly using an AI-powered search engine (MIT, 2023). 2.8k stars on GitHub.

_Source: https://olud.ai/project/gerevai-gerev.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
gerev

gerev

[Python](https://olud.ai/language/python.html)

## gerev

🧠 AI-powered enterprise search engine 🔎

by **GerevAI** · GitHub

[2.8k](https://olud.ai/most-starred.html)

Stars

[176](https://olud.ai/most-forked.html)

Forks

[MIT](https://olud.ai/license/mit.html)

License

[2023](https://olud.ai/year/2023.html)

Created

[2 yr ago](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Python](https://olud.ai/language/python.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [chatgpt](https://olud.ai/projects.html?cat=chatgpt)
- [confluence](https://olud.ai/projects.html?cat=confluence)
MIT
- [Python](https://olud.ai/language/python.html)

[View on GitHub](https://github.com/GerevAI/gerev)

𝕏  X / Twitter

🔗  Copy link

In plain wordsSearch for documents and conversations within your organization quickly using an AI-powered search engine.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=GerevAI%2Fgerev)

📈 Star history

2026-07-072026-08-31

📈 Track gerev

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

🧠 AI-powered enterprise search engine 🔎

gerev has 2.8k stars on GitHub. It has been forked 176 times. gerev is written mainly in Python. It has been in active development since 2023. gerev is available under the MIT license. Its main topics are ai, chatgpt, confluence, enterprise-search.

[Compare with the paid tools it replaces](https://olud.ai/alt/gerev.html)A free alternative to 
- [Glean](https://olud.ai/alternatives/glean.html)

📦 Latest release

❓ Frequently asked questions

#### What is gerev?
🧠 AI-powered enterprise search engine 🔎

#### Is gerev open source?
gerev is an open-source project. It is released under the MIT license.

#### Is gerev free?
Yes. gerev is free and open source — you can use, modify and self-host it.

#### What license does gerev use?
gerev is available under the MIT license.

#### What language is gerev written in?
gerev is written mainly in Python.

🏅 Maintainer of this project?

olud.ai badge — gerev
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=gerevai-gerev)](https://olud.ai/project/gerevai-gerev.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Shares DNA with[🧬 View the DNA map →](https://olud.ai/dna/enterprise-search.html)
- [ocular AI Powered Search and Chat for Orgs - Think ChatGPT meets Google Search but powered by your dat… ★ 445 · ai sharesenterprise-search](https://olud.ai/project/ocularengineering-ocular.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)

Measured from GitHub topics shared by both projects, weighted by how rare each topic is.
