# go-micro — A Go agent harness and service framework | olud.ai

> Build AI agents that can call tools, manage memory, and communicate with other agents using Go code (Apache-2.0, 2015). 23k stars on GitHub.

_Source: https://olud.ai/project/micro-go-micro.html_

---

[Home](https://olud.ai/)›
[Projects](https://olud.ai/projects.html)›
go-micro

go-micro

[Go](https://olud.ai/language/go.html)

## go-micro

A Go agent harness and service framework

by **micro** · GitHub

[23k](https://olud.ai/most-starred.html)

Stars

[2.4k](https://olud.ai/most-forked.html)

Forks

[+32 this week](https://olud.ai/trending.html)

Trending

[Apache-2.0](https://olud.ai/license/apache-2-0.html)

License

[2015](https://olud.ai/year/2015.html)

Created

[today](https://olud.ai/recently-updated.html)

Last commit

[ai](https://olud.ai/projects.html?cat=ai)

Category

[Go](https://olud.ai/language/go.html)

Language
- [ai](https://olud.ai/projects.html?cat=ai)
- [ai-agents](https://olud.ai/projects.html?cat=ai-agents)
- [distributed-systems](https://olud.ai/dna/distributed-systems.html)
Apache-2.0
- [Go](https://olud.ai/language/go.html)

[View on GitHub](https://github.com/micro/go-micro)

𝕏  X / Twitter

🔗  Copy link

In plain wordsBuild AI agents that can call tools, manage memory, and communicate with other agents using Go code.

### From the README
Install

```
docker run --rm -it micro/micro new helloworld
docker run --rm -it --network host -v "$(pwd)":/micro/helloworld micro/micro run
```

Excerpts from the project README on [GitHub](https://github.com/micro/go-micro). Copyright and licensing remain with the respective authors.

You maintain this project?

Claim its page: indexed whatever its rank, translated into six languages, and enriched with what you write yourself.

[Claim this page →](https://olud.ai/claim.html?repo=micro%2Fgo-micro)

📈 Star history

2026-06-282026-08-31

📈 Track go-micro

Get an email alert on its next release or when it starts trending — never miss the moment.

Free · no card · unsubscribe anytime

[Get email alerts →](https://olud.ai/account.html)

📄 About

A Go agent harness and service framework

go-micro has 23k stars on GitHub. It has been forked 2.4k times. go-micro is written mainly in Go. It has been in active development since 2015. go-micro is available under the Apache-2.0 license. Its main topics are ai, ai-agents, distributed-systems, go.

📦 Latest release

❓ Frequently asked questions

#### What is go-micro?
A Go agent harness and service framework

#### Is go-micro open source?
go-micro is an open-source project. It is released under the Apache-2.0 license.

#### Is go-micro free?
Yes. go-micro is free and open source — you can use, modify and self-host it.

#### What license does go-micro use?
go-micro is available under the Apache-2.0 license.

#### What language is go-micro written in?
go-micro is written mainly in Go.

🏅 Maintainer of this project?

olud.ai badge — go-micro
Add this live badge to your README — your GitHub stars and directory rank, refreshed daily.

`[![olud.ai](https://olud.ai/badge.php?tool=micro-go-micro)](https://olud.ai/project/micro-go-micro.html)`
Copy Markdown

[More badge options →](https://olud.ai/badge.html)

🧬 Related projects[🧬 View the DNA map →](https://olud.ai/dna/distributed-systems.html)
- [openclaw Your own personal AI assistant. Any OS. Any Platform. The lobster way. 🦞 ★ 383k · ai](https://olud.ai/project/openclaw-openclaw.html)
- [superpowers An agentic skills framework & software development methodology that works. ★ 28k · ai](https://olud.ai/project/obra-superpowers.html)
- [hermes-agent The agent that grows with you ★ 238k · ai](https://olud.ai/project/nousresearch-hermes-agent.html)
- [n8n Fair-code workflow automation platform with native AI capabilities. Combine visual building wit… ★ 203k · ai](https://olud.ai/project/n8n-io-n8n.html)
- [AutoGPT AutoGPT is the vision of accessible AI for everyone, to use and to build on. Our mission is to… ★ 187k · agentic-ai](https://olud.ai/project/significant-gravitas-autogpt.html)
- [firecrawl The context API to search, scrape, and interact with the web at scale. 🔥 ★ 174k · ai](https://olud.ai/project/firecrawl-firecrawl.html)
- [prompts.chat f.k.a. Awesome ChatGPT Prompts. Share, discover, and collect prompts from the community. Free a… ★ 168k · ai](https://olud.ai/project/f-prompts-chat.html)
- [stable-diffusion-webui Stable Diffusion web UI ★ 164k · ai](https://olud.ai/project/automatic1111-stable-diffusion-webui.html)
- [JavaGuide Java 面试 & 后端通用面试指南，覆盖计算机基础、数据库、分布式、高并发、系统设计与 AI 应用开发 ★ 157k · agent](https://olud.ai/project/snailclimb-javaguide.html)
- [dify Build Agentic workflows, RAG pipelines, with rich AI model and tool support on one collaborativ… ★ 154k · agent](https://olud.ai/project/langgenius-dify.html)
